<?php
	...
	class homeland_Widget_Property_Status extends WP_Widget {
		...
		public function __construct() {		
			$widget_ops = array(
				'classname' => 'homeland_widget-property-status', 
				'description' => esc_html__( 'Custom widget for property status', 'homeland' )
			);	
			parent::__construct( 'PropertyStatus', esc_html__( 'Homeland - Search By Status', 'homeland' ), $widget_ops );
		}
		...
		public function widget( $args, $instance ) {		
			...
			<ul>
				<?php
					...
					$homeland_status = get_categories( $args_status );						
				  	
			  	foreach( $homeland_status as $homeland_category ) :
			  		echo '<li><a href="' . esc_url( $homeland_advance_search_page_url ) . '?status='. $homeland_category->slug . '" title="' . sprintf( esc_html__( 'View all posts in %s', 'homeland' ), $homeland_category->name ) . '" ' . '>' . $homeland_category->name.'</a>&nbsp;('. $homeland_category->count .')</li>';
			  	endforeach;
				?>
			</ul><?php
		...
		public function form( $instance ) {				
			...
			?>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'homeland_posts_limit' ) ); ?>"><?php esc_html_e( 'Number of status to show:', 'homeland' ); ?>
					</label>
				...
				<p>	
					<small><i><?php esc_html_e( 'Property Status will automatically display', 'homeland' ); ?></i></small>	
				</p>
			<?php
		}		
